...
**還沒做的一件事：評估從來沒有真的跑過。**
`python -m scripts.evaluate_followup` 會呼叫真實 OpenAI，要花錢，
所以程式寫好了但數字還是空的。跑完之後 `docs/evaluation/` 會多一份報告，
那份數字才是可以寫進履歷的東西。
...
但**後端的 422 仍然要留著**，它守的是別的路：
電話進線、走進店裡、Sprint 8 的 n8n Webhook，這些來源的欄位不由我們決定。
兩者不衝突 —— 表單必填是「不要製造沒內容的客戶」，
422 是「真的沒內容時不要硬猜」。
