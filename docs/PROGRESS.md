...
`backend/evaluation/final_test.json`，15 筆，第一次也是唯一一次執行。
...
**要對外講數字就講 73.3%**（95% 信賴區間 48.0% ~ 89.1%，n=15）。

兩份資料集的差別只有一件事：holdout 是 v2 寫完之後才建立的，
出題的人知道規則長什麼樣；期末考的出題者從頭到尾沒讀過 prompt。
**所以那 22 個百分點就是「出題的人讀過 prompt」值多少分。**

原本〈方法上要留意的地方〉裡「holdout 也不是完全乾淨的」只是一句自我提醒，
現在它有數字了。如果沒有這份期末考，這個專案會一直宣稱 95.2%。

**捏造率是唯一沒有退步的指標**（0% → 0%）——
最不能妥協的那個，在沒見過的題目上沒有鬆動。

4 筆失敗裡有 3 筆是同一個問題：**地標型地名**（中興大學、逢甲大學周邊、
大里軟體園區）。prompt 從來沒有講過「大學、園區這種是地標但不是行政區的
地名該怎麼填」——因為出題的人是照著規則想題目，規則沒提到的東西也想不到。

**這些失敗不會拿去改 prompt。** 改了分數一定會變好看，但那個分數的意義
會消失，它會退化成第三個練習題，而且期末考只能用一次，補不回來。
完整的錯誤分析與後續處理方式記在
[evaluation/README.md](evaluation/README.md) 的〈期末考〉那一節。
